require "administrate/base_dashboard"

class DeliveryDashboard < Administrate::BaseDashboard
  # ATTRIBUTE_TYPES
  # a hash that describes the type of each of the model's fields.
  #
  # Each different type represents an Administrate::Field object,
  # which determines how the attribute is displayed
  # on pages throughout the dashboard.
  ATTRIBUTE_TYPES = {
    deliverable: Field::Polymorphic,
    pharmacy: Field::BelongsTo,
    id: Field::Number,
    created_at: Field::DateTime,
    updated_at: Field::DateTime,
    recipient_name: Field::String,
    recipient_phone_number: Field::String,
    recipient_address: Field::String,
    medications: Field::String,
    copay: Field::String,
    signature: Field::String,
    signed_on: Field::DateTime,
    patient_id: Field::Number,
    time: Field::String,
    matrix: Field::String,
    duration: Field::String,
    request_sent: Field::Boolean,
    signature_image: Field::Text,
    request_sent_on: Field::DateTime,
    completed: Field::Boolean,
    deleted: Field::Boolean,
    rx: Field::String,
  }.freeze

  # COLLECTION_ATTRIBUTES
  # an array of attributes that will be displayed on the model's index page.
  #
  # By default, it's limited to four items to reduce clutter on index pages.
  # Feel free to add, remove, or rearrange items.
  COLLECTION_ATTRIBUTES = [
    :deliverable,
    :pharmacy,
    :id,
    :created_at,
  ].freeze

  # SHOW_PAGE_ATTRIBUTES
  # an array of attributes that will be displayed on the model's show page.
  SHOW_PAGE_ATTRIBUTES = [
    :deliverable,
    :pharmacy,
    :id,
    :created_at,
    :updated_at,
    :recipient_name,
    :recipient_phone_number,
    :recipient_address,
    :medications,
    :copay,
    :signature,
    :signed_on,
    :patient_id,
    :time,
    :matrix,
    :duration,
    :request_sent,
    :signature_image,
    :request_sent_on,
    :completed,
    :deleted,
    :rx,
  ].freeze

  # FORM_ATTRIBUTES
  # an array of attributes that will be displayed
  # on the model's form (`new` and `edit`) pages.
  FORM_ATTRIBUTES = [
    :deliverable,
    :pharmacy,
    :recipient_name,
    :recipient_phone_number,
    :recipient_address,
    :medications,
    :copay,
    :signature,
    :signed_on,
    :patient_id,
    :time,
    :matrix,
    :duration,
    :request_sent,
    :signature_image,
    :request_sent_on,
    :completed,
    :deleted,
    :rx,
  ].freeze

  # Overwrite this method to customize how deliveries are displayed
  # across all pages of the admin dashboard.
  #
  # def display_resource(delivery)
  #   "Delivery ##{delivery.id}"
  # end
end
